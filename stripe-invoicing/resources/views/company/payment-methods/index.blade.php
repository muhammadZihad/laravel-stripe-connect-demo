            ...
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p class="text-gray-600">Manage how you pay invoices</p>
                    </div>
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg p-6 {{ $paymentMethod->is_default ? 'ring-2 ring-primary-500' : '' }}">
                                <div class="flex items-center justify-between mb-4">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        @if(!$paymentMethod->is_default)
                                            <form method="POST" action="{{ route('company.payment-methods.set-default', $paymentMethod) }}" class="inline">
                                                @csrf
                                        ...
                                        @endif
                                        <form method="POST" action="{{ route('company.payment-methods.delete', $paymentMethod) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this payment method?')">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="space-y-3">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500">
                                            ...
                                            <br>
                                            <span class="inline-flex items-center px-2 py-1 text-xs font-medium rounded-full {{ $paymentMethod->verification_badge['class'] ?? 'bg-gray-100 text-gray-800' }}">
                                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                    @if(isset($paymentMethod->verification_badge['icon']) && $paymentMethod->verification_badge['icon'] === 'check-circle')
                                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                                    @elseif(isset($paymentMethod->verification_badge['icon']) && $paymentMethod->verification_badge['icon'] === 'exclamation-triangle')
                                                        <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/>
                                                    @elseif(isset($paymentMethod->verification_badge['icon']) && $paymentMethod->verification_badge['icon'] === 'clock')
                                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clip-rule="evenodd"/>
                                                    @elseif(isset($paymentMethod->verification_badge['icon']) && $paymentMethod->verification_badge['icon'] === 'x-circle')
                                    ...
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            @if($paymentMethod->type === 'us_bank_account')
                                                @if(method_exists($paymentMethod, 'requiresVerification') && $paymentMethod->requiresVerification())
                                                    <button onclick="initiateVerification({{ $paymentMethod->id }})" 
                                                            class="text-blue-600 hover:text-blue-900 text-sm font-medium">
                                                        Start Verification
                                                    </button>
                                                @elseif(method_exists($paymentMethod, 'isPendingVerification') && $paymentMethod->isPendingVerification())
                                                    <a href="{{ route('company.payment-methods.verify', $paymentMethod) }}" 
                                                       class="text-blue-600 hover:text-blue-900 text-sm font-medium">
                                                        Enter Verification
                                                    </a>
                                                @elseif(method_exists($paymentMethod, 'verificationFailed') && $paymentMethod->verificationFailed())
                                                    @if(method_exists($paymentMethod, 'canAttemptVerification') && $paymentMethod->canAttemptVerification())
                                                        <button onclick="initiateVerification({{ $paymentMethod->id }})" 
                                            ...
                                            @if(!$paymentMethod->is_default && $paymentMethod->is_active)
                                                <form action="{{ route('company.payment-methods.set-default', $paymentMethod) }}" method="POST" class="inline">
                                                    @csrf
                                            ...
                                            <!-- Delete Action -->
                                            <form action="{{ route('company.payment-methods.delete', $paymentMethod) }}" method="POST" class="inline" 
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <p class="mt-1 text-sm text-gray-500">You haven't added any payment methods yet. Add one to start paying invoices.</p>
                        <div class="mt-6">
                            <a href="{{ route('company.payment-methods.create') }}" class="bg-primary-600 hover:bg-primary-700 text-white px-4 py-2 rounded-md text-sm font-medium">
                        ...
                        <div class="ml-3">
                            ...
                            <div class="mt-2 text-sm text-blue-700">
                                <ul class="list-disc list-inside space-y-1">
                                    <li>Payment methods are used to pay invoices from your agents</li>
                                    <li>Your default payment method will be used for all new payments</li>
                                    <li>All payments are processed securely through Stripe</li>
                                    <li>You can pay invoices quickly using saved payment methods</li>
                                    <li>Charges are typically processed immediately</li>
                                </ul>
                            </div>
                        </div>
                    </div>
...
async function initiateVerification(paymentMethodId) {
    ...
    try {
        const response = await fetch(`/company/payment-methods/${paymentMethodId}/initiate-verification`, {
            method: 'POST',
            headers: {
