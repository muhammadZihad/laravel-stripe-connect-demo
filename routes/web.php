...
Route::post('/invoice/pay/{token}/confirm', [StripeController::class, 'confirmPublicInvoicePayment'])->name('invoice.public.confirm');
...
Route::middleware(['auth', 'super_admin'])->prefix('super-admin')->name('super_admin.')->group(function () {
    ...
    Route::get('/transactions/pending/list', [SuperAdminController::class, 'pendingTransactions'])->name('transactions.pending');
    Route::post('/transactions/pending/reconcile', [SuperAdminController::class, 'reconcilePendingTransactions'])->name('transactions.reconcile');
    Route::get('/transactions/{transaction}/check-status', [SuperAdminController::class, 'checkTransactionStatus'])->name('transactions.check-status');
...
Route::middleware(['auth', 'company'])->prefix('company')->name('company.')->group(function () {
    ...
    Route::post('/invoices/{invoice}/pay/confirm', [CompanyController::class, 'confirmPayment'])->name('invoices.pay.confirm');
