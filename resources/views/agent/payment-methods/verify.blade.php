            ...
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p class="text-gray-600">Choose your preferred verification method for your bank account</p>
                    </div>
                    <div class="flex gap-3">
                        <a href="{{ route('agent.payment-methods') }}" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-md text-sm font-medium">
                ...
                <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    <h3 class="text-sm font-medium text-blue-800 mb-2">Bank Account Details</h3>
                    <div class="text-sm text-blue-700 space-y-1">
                        <p><strong>Bank:</strong> {{ $paymentMethod->bank_name ?? 'N/A' }}</p>
                        <p><strong>Account Type:</strong> {{ ucfirst($paymentMethod->account_holder_type ?? 'N/A') }}</p>
                        <p><strong>Last 4 Digits:</strong> •••• {{ $paymentMethod->last_four }}</p>
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $paymentMethod->verification_badge['class'] }}">
                        ...
                        @if($paymentMethod->verification_initiated_at)
                            <p><strong>Verification Started:</strong> {{ $paymentMethod->verification_initiated_at->format('M j, Y g:i A') }}</p>
                        @endif
                    </div>
                </div>
                ...
                <div id="microdeposit-form-section" style="display: none;">
                    <form id="verification-form" method="POST" action="{{ route('agent.payment-methods.verify.submit', $paymentMethod) }}">
                        @csrf
                ...
                <!-- Submit Button -->
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex gap-3">
                                <a href="{{ route('agent.payment-methods') }}" class="bg-gray-300 hover:bg-gray-400 text-gray-700 px-6 py-2 rounded-md text-sm font-medium">
                                    Cancel
                                </a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    startVerificationBtn.addEventListener('click', async function() {
        ...
        try {
            const response = await fetch('{{ route('agent.payment-methods.initiate-verification', $paymentMethod) }}', {
                method: 'POST',
                headers: {
    ...
    async function pollForVerificationCompletion() {
        ...
        const poll = async () => {
            ...
            try {
                const response = await fetch('{{ route('agent.payment-methods.check-status', $paymentMethod) }}', {
                    method: 'GET',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                        'Accept': 'application/json',
